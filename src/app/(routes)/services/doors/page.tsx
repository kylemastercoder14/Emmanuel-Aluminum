...
const Page = async () => {
  ...
  return (
	<div className="p-10 px-20 mt-20 mb-20 min-h-screen">
	  <h3 className="text-2xl font-bold tracking-tight">Door Design</h3>
	  <p className="text-muted-foreground">
		Browse all <span className="font-semibold">door design</span>{" "}
		services.
	  </p>
	  <div className="grid mt-5 lg:grid-cols-4 grid-cols-1 gap-5">
		{data.length > 0 ? (
		  data.map((service) => (
			<Link
			  href={`/services/doors/${service.id}`}
			  key={service.id}
		  ...
		  <div className="col-span-4 flex flex-col mt-10 items-center justify-center text-center">
			...
			<p className='mt-5 text-xl font-semibold'>No door design services found.</p>

...
const Page = async () => {
  ...
  return (
	<div className="p-10 px-20 mt-20 mb-20 min-h-screen">
	  <h3 className="text-2xl font-bold tracking-tight">Cabinet Design</h3>
	  <p className="text-muted-foreground">
		Browse all <span className="font-semibold">cabinet design</span>{" "}
		services.
	  </p>
	  <div className="grid mt-5 lg:grid-cols-4 grid-cols-1 gap-5">
		{data.length > 0 ? (
		  data.map((service) => (
			<Link
			  href={`/services/cabinets/${service.id}`}
			  key={service.id}
		  ...
		  <div className="col-span-4 flex flex-col mt-10 items-center justify-center text-center">
			...
			<p className='mt-5 text-xl font-semibold'>No cabinet design services found.</p>
			<p className='mt-2 text-muted-foreground'>Please refresh the page, or check back later.</p>
		  </div>
		)}
	  </div>
	</div>

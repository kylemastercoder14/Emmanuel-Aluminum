...
import { payService } from "@/actions/order";
...
const OrderHistoryPage = ({ orders }: { orders: OrderWithOrderItems[] }) => {
  ...
  const openPayModal = (order: OrderWithOrderItems) => {
    // prefill attachments from order
    setPaymentAttachments(order.paymentAttachment || []);
  ...
  const submitPayment = async () => {
    ...
    const response = await payService({
      ...
      note: "Customer submitted proof of payment.",
    });
  ...
  return (
    <>
      <Modal isOpen={isPayOpen} onClose={() => setIsPayOpen(false)}>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label>Proof of Payments</Label>
            <MultipleImageUpload
              onUploadComplete={(urls) => setPaymentAttachments(urls)}
              defaultValues={paymentAttachments
              ...
              <div className="flex flex-col md:flex-row text-left md:text-right gap-2 md:justify-end">
                {(order.status === "Completed" || order.status === "Scheduled") && (
                  <Button
                    size="sm"
                    variant="ghost"
                    className="bg-green-300/50 border-green-600 hover:text-green-600 hover:bg-green-600/20 text-green-600"
                ...
                <Button
                  ...
                  onClick={() =>
                    router.push(`/service-history/${order.orderId}`)
                  }
                >
                  View Details
                </Button>

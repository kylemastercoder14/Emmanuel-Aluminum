...
export function DataTable<TData, TValue>({
  ...
  React.useEffect(() => {
    const timeout = setTimeout(() => {
      table.getColumn("id")?.setFilterValue(search);
    }, 100);
    ...
    const fetchSuppliers = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          const supplierOptions = result.data.map((supplier: any) => ({
            label:
              supplier.name ||
  ...
  return (
    <div>
      <div className="flex items-center py-4">
        ...
        />
        {table.getColumn("supplierId") && (
          <DataTableFilter
            options={suppliers}
            column={table.getColumn("material") as Column<TData, unknown>}
            title="Supplier"
          />
        )}
        <DropdownMenu>
